...
export default function EditProduct() {
  ...
  const fetchProduct = async () => {
    try {
      const response = await axios.get(`/products/getOneProduct/${id}`);
      ...
      setAdSpendingHistory(product.adSpendingHistory || []);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(`/products/updateProduct/${id}`, productData);
      toast.success('تم تحديث المنتج بنجاح');
      navigate('/products');
    } catch (error) {
      const message = error.response?.data?.message || 'حدث خطأ في تحديث المنتج';
      toast.error(message);
    } finally {
      setSaving(false);
    }
      ...
      <Paper sx={{ p: 3 }}>
        <Box component="form" onSubmit={handleSubmit}>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            <Typography variant="h6">تاريخ إنفاق الإعلانات</Typography>
            <Button
              variant="outlined"
              startIcon={<AddIcon />}
